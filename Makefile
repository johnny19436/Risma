...
HOMEBREW_LIB := -L/opt/homebrew/lib -lboost_math_c99 -ltbb
...
ifeq ($(UNAME_S),Darwin)
    ...
    # Add OpenMP flags for macOS (requires libomp from Homebrew)
...
# CXXFLAGS with OpenMP support
CXXFLAGS := -O2 -g -fno-omit-frame-pointer -std=c++17 -fPIC $(OPENMP_FLAGS)
INCLUDES := -I$(PYBIND11_INCLUDE) -I$(BLAS_INCLUDE) -I$(HOMEBREW_INCLUDE) $(PYTHON_INCLUDE) 
LDFLAGS := $(PYTHON_LDFLAGS) $(BLAS_LIB) $(HOMEBREW_LIB) $(OPENMP_LIBS)
